...
import type { RideStats, TimeDistribution, SeasonalStats, WeekdayStats } from '../types/stats';
...
export function calculateRideStats(activities: StravaActivity[]): RideStats {
  ...
  const longestRide = activities.reduce((max, current) => 
    current.distance > max.distance ? current : max, activities[0]
  );

  const highestClimb = activities.reduce((max, current) => 
    current.total_elevation_gain > max.total_elevation_gain ? current : max, activities[0]
  );

  const fastestRide = activities.reduce((max, current) => 
    current.max_speed > max.max_speed ? current : max, activities[0]
  );
...
export function calculateSeasonalStats(activities: StravaActivity[]): SeasonalStats {
