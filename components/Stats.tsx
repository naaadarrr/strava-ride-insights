...
export function Stats({ activities }: StatsProps) {
  ...
  const years = [...new Set(activities.map(a => new Date(a.start_date).getFullYear()))].sort(
    (a, b) => b - a
  )
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-orange-50 p-4 rounded-lg">
          ...
          <p className="text-sm text-orange-600 mt-1">
            {t('stats.longest_ride')}: {(stats.longest_ride / 1000).toFixed(1)}{' '}
            {t('stats.kilometers')}
          </p>
        </div>
        ...
        <div className="bg-blue-50 p-4 rounded-lg">
          ...
          <p className="text-2xl font-bold text-blue-700">{formatDuration(stats.total_time)}</p>
        ...
        <div className="bg-purple-50 p-4 rounded-lg">
          ...
          <p className="text-sm text-purple-600 mt-1">{t('stats.fastest_speed_record')}</p>
        </div>
      </div>
    </div>
  )
