...
export function useStatsActivities() {
  ...
  const fetchActivitiesPage = useCallback(async (page: number) => {
    const response = await fetch(`/api/strava/activities?per_page=${PAGE_SIZE}&page=${page}`)
  ...
  useEffect(() => {
    ...
    const fetchAllActivities = async () => {
      ...
      try {
        ...
        while (true) {
          const data = await fetchActivitiesPage(page)
